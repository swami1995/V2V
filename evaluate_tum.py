...
def evaluate_tum(tum_scenes, cfg, args, ckpt):
    results = {}
    for scene in tum_scenes:
        scene_dir = args.tumdir / f"rgbd_dataset_freiburg1_{scene}"
        ...
        for trial_num in range(args.trials):
            traj_est, timestamps = run(cfg, ckpt, scene_dir, scene, args.stride, args.viz)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tumdir', type=Path, default="/project_data/datasets/tum-rgbd")
    ...
    ckpt = args.network
    save_str = ckpt.split('/')[-1].replace('.pth', f'tum_trials{args.trials}.txt')
    results = evaluate_tum(tum_scenes, cfg, args, ckpt)
    np.save('tum_results/' + save_str, results)
